...
if is_bnb_available():
    ...
    class Linear8bitLt(bnb.nn.Linear8bitLt, LoraLayer):
        ...
        def forward(self, x: torch.Tensor, instance_embed=None):
            ...
            if self.disable_adapters or self.active_adapter not in self.lora_A.keys():
                ...
                if instance_embed is not None:
                    gate = self.lora_gate(instance_embed).reshape(-1, self.r[self.active_adapter], self.r[self.active_adapter])
                ...
                if not torch.is_autocast_enabled():
                    ...
                    A_output = self.lora_A[self.active_adapter](self.lora_dropout[self.active_adapter](x))
                    output = (
                        self.lora_B[self.active_adapter](
                            torch.bmm(A_output, gate) if instance_embed is not None else A_output
                        ).to(expected_dtype)
                        * self.scaling[self.active_adapter]
                    )
                else:
